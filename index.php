...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --bg-skill: rgba(25, 30, 38, 0.7);
            --bg-input: #1A1E26;
            --border-color: #2A2E3A;
            --text-primary: #E5E9F0;
            --text-secondary: #B7C2DA;
            --text-muted: #8F9BB3;
            --glow-color: rgba(108, 99, 255, 0.3);
            --shadow-color: rgba(0, 0, 0, 0.5);
            --card-hover-border: #6C63FF;
            --gradient-hero: linear-gradient(135deg, #F0F3FF, #B9B4FF, #A970FF);
            --gradient-heading: linear-gradient(135deg, #FFFFFF 0%, #B9B4FF 100%);
            --toggle-bg: #1E1F2C;
            --toggle-icon: #C084FC;
        ...
        [data-theme="light"] {
            ...
            --bg-skill: rgba(240, 242, 248, 0.8);
            --bg-input: #FFFFFF;
            --border-color: #D0D5E0;
            --text-primary: #1A1E2C;
            --text-secondary: #2D3346;
            --text-muted: #5A6378;
            --glow-color: rgba(108, 99, 255, 0.2);
            --shadow-color: rgba(0, 0, 0, 0.08);
            --card-hover-border: #6C63FF;
            --gradient-hero: linear-gradient(135deg, #2D1B69, #6C63FF);
            --gradient-heading: linear-gradient(135deg, #1A1E2C 0%, #6C63FF 100%);
            --toggle-bg: #D0D5E0;
            --toggle-icon: #6C63FF;
        ...
        * {
        ...
        .particle {
            position: absolute;
            border-radius: 50%;
            background: var(--particles-color1);
            animation: floatParticle linear infinite;
            box-shadow: 0 0 6px var(--particles-color1);
        }
...
<div class="container main-container">
            ...
            <div class="row g-3 align-items-center">
                <div class="col-md-8">
                    <p style="color: var(--text-secondary);">
                        <i class="fas fa-code me-2"></i> Este ejemplo consume el servicio SOAP público de 
                        <strong>conversión de temperatura</strong> (Celsius ↔ Fahrenheit). 
                        Introduce un valor en grados Celsius y obtén su equivalente en Fahrenheit.
                    </p>
                    <div class="d-flex gap-3 flex-wrap">
                        <input type="number" id="celsiusInput" class="form-control" style="max-width: 150px;" placeholder="°C" value="25">
                        <button class="btn btn-primary" id="soapConvertBtn" style="background:#6C63FF; border:none;">
                            <i class="fas fa-arrow-right"></i> Convertir vía SOAP
                        </button>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="result-box" id="soapResult">
                        <span class="text-muted">Resultado aparecerá aquí</span>
                    </div>
                </div>
            </div>
            <div class="mt-3">
                <small class="text-muted">
                    <i class="fas fa-info-circle"></i> Servicio: 
                    <a href="https://www.w3schools.com/xml/tempconvert.asmx?wsdl" target="_blank" style="color: #6C63FF;">
                        tempconvert.asmx
                    </a> 
                    (público, usando proxy CORS)
                </small>
            </div>
        </div>
    </section>
    ...
    <!-- Footer -->
    <footer class="footer" id="dynamicFooter"></footer>
</div>
...
<div class="chatbot-container">
    ...
    <div class="chat-window" id="chatWindow">
        <div class="chat-header">
            <span><i class="fas fa-comment-dots"></i> Asistente IA · Resuelve dudas</span>
        ...
        <div class="chat-input-area">
            ...
            <button class="chat-send" id="chatSendBtn"><i class="fas fa-arrow-up"></i></button>
        </div>
    </div>
</div>
...
<script>
    ...
    function renderSkills() {
        ...
        skillsWithIcons.forEach(skill => {
            ...
            div.innerHTML = `<i class="${skill.icon}"></i><span class="skill-name">${skill.name}</span>`;
            col.appendChild(div);
    ...
    document.getElementById('soapConvertBtn').addEventListener('click', function() {
        ...
        if (!celsius || isNaN(celsius)) {
            document.getElementById('soapResult').innerHTML = '<span style="color: #ff6b6b;">❌ Ingresa un número válido</span>';
            return;
        }

        const resultBox = document.getElementById('soapResult');
        resultBox.innerHTML = '<span class="text-muted">⏳ Enviando petición SOAP...</span>';
        ...
        .then(xmlText => {
            ...
            const resultNode = xmlDoc.getElementsByTagName('CelsiusToFahrenheitResult')[0];
            if (resultNode) {
                const fahrenheit = resultNode.textContent.trim();
                resultBox.innerHTML = `
                    <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
                        <span style="font-weight:600; color:var(--text-primary);">🌡️ Resultado:</span>
                        <span style="font-size:1.2rem; color:#6C63FF;">${celsius}°C = ${fahrenheit}°F</span>
                    </div>
                `;
            } else {
    ...
    function openChat() { chatWindow.style.display = 'flex'; }
    function closeChatWindow() { chatWindow.style.display = 'none'; }
